...
{
    public partial class CBT02220 : R_Page
    {
        ...
        private PageParameterDTO loPageParameter = null;
        ...
        protected override async Task R_Init_From_Master(object poParameter)
        {
            ...
            try
            {
                //loPageParameter = (PageParameterDTO)poParameter;
                loPageParameter = R_FrontUtility.ConvertObjectToObject<PageParameterDTO>(poParameter);
                loChequeEntryViewModel.loPageParameter = loPageParameter;
                loChequeEntryViewModel.IsOpenAsPage = true;
                ...
                if (loPageParameter.PARAM_CALLER_ACTION == VIEW_ONLY)
                {
                    IsHidden = true;
                    await _conductorRef.R_GetEntity(new CBT02210DTO()
                    {
                        CREC_ID = loPageParameter.CCHEQUE_ID,
                        CDEPT_CODE = loPageParameter.PARAM_DEPT_CODE,
                        CREF_NO = loPageParameter.PARAM_REF_NO,
                    });
                }
                else if (loPageParameter.PARAM_CALLER_ID == "PMT05500")
                {
                    switch (loPageParameter.PARAM_CALLER_ACTION)
                    {
                        case NEW:
                ...
                //if (!string.IsNullOrWhiteSpace(loPageParameter.CCHEQUE_ID))
                //{
                //    await _conductorRef.R_GetEntity(new CBT02210DTO()
                //    {
                //        CREC_ID = loPageParameter.CCHEQUE_ID
        ...
        private async Task BtnDelete_OnClick()
        {
            ...
            try
            {
                ...
                loParam.CREC_ID_LIST = loPageParameter.CCHEQUE_ID;
        ...
        private async Task ChequeForm_AfterAdd(R_AfterAddEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                loData.CDEPT_CODE = loPageParameter.PARAM_DEPT_CODE;
                //await DeptCode_OnLostFocus(loData.CDEPT_CODE);
                loData.NTRANS_AMOUNT = loPageParameter.PARAM_AMOUNT;
                loData.CTRANS_DESC = loPageParameter.PARAM_DESCRIPTION;
