...
namespace CBT00200FRONT
{
    public partial class CBT00210 : R_Page
    {
        ...
        private async Task JournalForm_AfterAdd(R_AfterAddEventArgs eventArgs)
        {
            ...
            try
            {
                if (_JournalEntryViewModel.FlagIsCopy)
                {
                    eventArgs.Data = R_FrontUtility.ConvertObjectToObject<CBT00200DTO>(_JournalEntryViewModel.Journal);
                    var data = (CBT00200DTO)eventArgs.Data;
                    data.CREF_NO = "";

                    _JournalEntryViewModel.RefDate = DateTime.ParseExact(_JournalEntryViewModel.Journal.CREF_DATE, "yyyyMMdd", CultureInfo.InvariantCulture);
                    _JournalEntryViewModel.DocDate = DateTime.ParseExact(_JournalEntryViewModel.Journal.CDOC_DATE, "yyyyMMdd", CultureInfo.InvariantCulture);
                    _JournalEntryViewModel.FlagIsCopy = false;
        ...
        {
            var lodata = _gridDetailRef.CurrentSelectedData;
        ...
        private void JournalDet_Validation(R_ValidationEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                {
                    var loDataList = _gridDetailRef.DataSource.ToList();
                    var loindexData = loDataList.FindIndex(x => x.INO == loData.INO);
                    loDataList.RemoveAt(loindexData);

                    if (loDataList.Any(x => x.CGLACCOUNT_NO == loData.CGLACCOUNT_NO))
                    {
                        loEx.Add(R_FrontUtility.R_GetError(
                            typeof(Resources_Dummy_Class),
                            "V23"));
        ...
        {
            EnableDetailNormalMode = eventArgs.Enable;
        }
        private void JournalDet_CheckGridAdd(R_CheckGridEventArgs eventArgs)
        {
            eventArgs.Allow = EnableHaveRecId && EnableEdit;
        }
        ...
        private async Task CopyJournalEntryProcess()
        {
            ...
            try
            {
                _JournalEntryViewModel.FlagIsCopy = true;
